...
try {
  ...
  const routes=['/','/topics/','/series/','/library/','/notes/','/about/',...registry.topics.map(x=>`/topics/${x.id}/`),...registry.series.map(x=>`/series/${x.id}/`)];
  ...
  await page.locator('[name="series"]').selectOption('agent-evaluation');
  await page.waitForFunction(()=>document.querySelectorAll('.library-result').length===5);
  ...
  await page.reload();await page.locator('.library-controls').waitFor({state:'visible'});assert.equal(await page.locator('[name="series"]').inputValue(),'agent-evaluation');
  await page.locator('[name="topic"]').selectOption('practice');await page.locator('.library-empty').waitFor({state:'visible'});
  await page.goBack();await page.waitForFunction(()=>document.querySelectorAll('.library-result').length===5);assert.equal(await page.locator('[name="topic"]').inputValue(),'');
  ...
  await visit('/library/?tag=UnknownTag');await page.locator('.library-empty').waitFor({state:'visible'});assert.equal(await page.locator('[name="tag"]').inputValue(),'UnknownTag');
  await page.locator('[data-reset]').first().click();await page.locator('#library-query').press('Tab');assert.equal(await page.evaluate(()=>document.activeElement.textContent),'搜索 →');
  await visit('/essays/');await page.waitForURL('**/library/?type=essay');await page.locator('.library-controls').waitFor({state:'visible'});assert.equal(await page.locator('[name="type"]').inputValue(),'essay');
  console.log('PASS search, filters, series order, URL/reload/back, reset, empty state, pagination, keyboard and legacy entry');checks+=12;
  ...
  for(const width of [390,768,1024]) {
    ...
    for(const route of ['/','/topics/','/series/agent-evaluation/','/library/','/writing/agent-evaluation-metrics/']) {
  ...
  if(screenshots) {
    ...
    for(const [name,route] of [['home','/'],['directory','/topics/'],['series','/series/agent-evaluation/'],['library','/library/'],['article','/writing/agent-evaluation-metrics/']]) {
