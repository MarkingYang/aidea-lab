## Publication authorization — concise LLM API lifecycle, 2026-09-17

The user explicitly requests optimizing the diagram and article for concise, focused explanation, then publishing. Publish the 10-minute `llm-api-call-lifecycle.md`, its existing model directory entry and `docs/llm-api-call-study/` through origin/main. One opening Mermaid overview replaces the earlier three diagrams; six sections replace nine, and formulas, capacity arithmetic and secondary mechanism comparisons are removed. Distinguish application context from service input preparation, KV state from execution steps, streaming during generation, complete tool requests with permission checks and a real return path, normal completion from partial/error output, and cross-cutting security/metering. Keep official sources and hypothetical, unrun-example boundaries. The isolated publication has 78 articles, 24 groups and 8 domains, with 147 graph nodes and 624 links, based on the separately published API/Runtime article at fb8a95a. Exclude unrelated homepage edits, unpublished interview, archives and WeChat files. Earlier local-only lifecycle notes describe superseded drafts; later additions need new publication authorization.
...
## Latest local article — Dify and Workflow agents, 2026-09-16

The user requests one accessible but substantive Blog starting from Dify, explaining Workflow-based agents and their significance as autonomous agents improve. Added `dify-workflow-agents.md` in the existing Agent product-design group, with a hypothetical after-sales case and two editable Mermaid diagrams. Official-source research is scoped to current Dify Cloud documentation, including the new Agent node's Beta / Workflow-only boundary; it is not a self-hosted version guarantee. Sources and site validation are in `docs/dify-workflow-study/`. No Dify instance, real model, Agent or business action was executed. Local content is 77 articles, 24 groups and 8 domains; graph is 145 nodes and 617 links. Build and desktop/mobile/theme diagram checks passed. This addition remains local and requires a new publication request; do not generate WeChat adaptations.

## Timeline wording publication and WeChat handoff — 2026-09-16

The user reviewed two small Blog wording edits and explicitly requested republishing: title subsequently refined during publication to “从 Transformer 到 Managed Agents：九年，大模型走向自主执行的完整脉络”, and removal of “回头看，” from the third opening paragraph. Synchronize the title metadata and both article copies; preserve the URL, publication date, 46 historical nodes, timeline and images. The opening still anchors the nine years to the 2017 Transformer paper. The user now maintains the WeChat account manually: stop generating, synchronizing or publishing WeChat adaptations unless explicitly requested again. Retain local `wechat/` artifacts and `/wechat/` in `.gitignore`; do not commit that directory. Exclude the unrelated Yang Zhilin interview and archives from this publication.
...
## Local image paths — Obsidian compatibility, 2026-09-16

The user reads `docs/llm-to-agent-evolution.md` in the aidea-lab Obsidian vault. Keep HTML image sources relative to each Markdown file (`../public/images/…` in docs; `../../../public/images/…` in writing). `remark-public-images` maps these to website root paths at build time, including the development preview. Preserve this deliberate path difference when synchronizing bodies. Inline proportional sizing is needed in Obsidian; source assets remain unchanged. See `docs/markdown-guide.md`.

## Latest local revision — six historical stage images, 2026-09-16

The user explicitly requests one sourced image for each of the six historical stages, preferring existing originals downloaded locally and generation only if none are suitable. This is an authorized exception to the default no-images rule for this article. Six unaltered source assets are stored in `public/images/llm-to-agent-evolution/`, with captions, attribution and source links in both article copies. Their provenance, rights statements and hashes are in `docs/timeline-publication/stage-images.json`. The 2022 screenshot is dated December 5, 2022, and carries the uploader's CC BY-SA 4.0 attribution; official vendor illustrations are not labeled as openly licensed. Preserve all narrative text, dates, diagrams and milestones. This revision remains local until a new publication request.

## Latest local revision — documentary narrative, 2026-09-16

The user rejects the rigid per-node explanatory template and requests immersive documentary-style Chinese narration. This supersedes the earlier requirement to display five labeled fields: retain their factual coverage in continuous prose, weaving illustrative scenarios into the account of events, mechanisms, limits and subsequent developments. Preserve all 46 dated nodes, source URLs, twelve overview milestones and the ReAct diagram. Keep hypothetical examples distinguishable from actual research scenes and vendor demos; do not invent historical scenes or imply direct lineage without evidence. The user-edited title is “回望 LLM 诞生的九年：从 LLM 到 Agent”, with the opening explicitly anchoring the period to Transformer in 2017. Draft and article are synchronized. These post-publication revisions remain local until a new publication request.

## Publication authorization — timeline milestones and writing voice, 2026-09-16

The user asks to combine memorable historical events with technical depth and reflective interpretation, in their own retrospective Chinese writing voice (“回望过去…”). Preserve the chronological knowledge structure and five explanatory fields; use concrete releases, demonstrations and research scenes to introduce mechanisms and remaining problems. The timeline now retains all original 38 nodes and source URLs, adds eight event nodes (46 total), and uses twelve linked milestones in the horizontal overview. The title is “回望过去的九年：从 LLM 到 Agent”; its URL and original publication date remain unchanged. Anchor the nine-year period to the 2017 Transformer paper rather than asserting that all language-model history began then. Publication demos and vendor reports are not independent validation; later engineering disclosures are identified as later evidence. Draft and formal article body are synchronized. The user explicitly requested “发布” after reviewing this revision, authorizing this article and its supporting records through origin/main. Unrelated local drafts are excluded; subsequent additions require a new publication request. See `docs/timeline-publication/milestone-revision.md`.
...
## Latest local continuation — Agent architecture, data and evaluation, 2026-09-09

The user requested supplementing Agent architecture design, data and evaluation. Add one coherent `agent-data-architecture.md` article; deepen the original Harness architecture and evaluation articles with constraint-driven boundaries, online/offline feedback, frozen experiment manifests, evidence-aware grading and judge calibration. Current local content is 73 articles, 24 groups and 8 domains; graph has 139 nodes and 580 links. Six new editable diagrams accompany the text. `docs/agent-data-evaluation/` preserves official-source response hashes, sixteen Python/SQLite contract checks and four fixed candidate results. These do not run models, real Agents, model judges, training, OS isolation or distributed delivery. Fixture costs and rates are teaching arithmetic, not measured Agent performance. Preserve original publication dates, Lu Qi and previous experiment/source records. This batch and the prior architecture-rationale batch remain local until a new publication request.

## Latest local continuation — architecture rationale, 2026-09-08

The user requested continuing the assessment of whether existing articles and GitHub studies explain the architecture and its design reasons. Deepen the existing Codex article at its original URL using fixed commit `4b0d9669cc46ba97bf85fa6312431b630d80498d`; connect its source findings to Harness and Agent Runtime. Add the same-ticket-task design rationale and eight paired counterexamples to the architecture selection article. Preserve source hashes and static-only Codex boundaries in `docs/codex-source-study/`; the sixteen Python/SQLite executions in `docs/architecture-rationale/` use fixed responses and do not execute Codex or the A/B/C engines. Keep 72 articles, 24 groups and 8 domains; the graph now has 138 nodes and 565 links from added references. Lu Qi, original publication dates and historical experiment results are unchanged. This new batch is local only and requires a new publication request.
...
## Latest user request — open-source Harness research, 2026-09-07

The user asks to supplement Harness knowledge through open-source projects. Locally deepen the single LangGraph article and add one complete article each for Temporal (Python SDK scope) and OpenHands Software Agent SDK. Current local count is 58 articles, 22 groups and 8 domains; the prior published baseline is `0dce1cc`. Keep immutable source commits, code paths, experiments and limitations in `docs/harness-source-study/`. The continuation deepens the existing OpenCode and Kimi Code articles and links their source evidence into context/tool mechanism articles. Keep those commits, nine module experiments and limitations in `docs/context-tool-study/`; that continuation kept 57 articles. The next continuation adds one complete Haystack article, connects it to retrieval/context articles, and records eleven real Pipeline experiments in `docs/haystack-source-study/`; the current count is 58. This new content is not authorized for publication. Do not treat the prior 55-article publication authorization as permission to push these additions.

## Latest user request — object-level depth, 2026-09-07

The user corrected the 17-article series merge: each GitHub research project needs one complete article, and each coherent knowledge problem needs deep, high-density analysis. Primary directions are Harness, models, technical architecture, products, competitive analysis and technology selection; evaluation and research perspectives supplement these. Categories organize articles and do not determine article count. Do not merge different projects into one survey, fragment a single project's runtime/state/extensions, or pad text to reach 20–30 minutes. Research objects independently. Current local restructuring has 55 articles, 22 directory groups and 8 domains; these are not future quotas. Source depth varies: structure checks do not certify every article as a completed deep study. See `docs/object-research/README.md`, `articles.md` and `manifest.json` for boundaries, evidence gaps and mapping. The user explicitly authorized publishing this 55-article version on 2026-09-07 through origin/main. Future edits still require a new publish request. This instruction supersedes conflicting consolidation and historical restoration instructions below.

## Superseded request — series consolidation, 2026-09-07

The user now requests merging the articles in each existing series into one coherent article with a 20–30 minute reading target. Local content contains 17 articles (one per series), merged from the published 88-article baseline `d5e12d0`; historical restoration instructions below are superseded for this edit. Preserve original publication dates, record the current revision date, remove article images under the default rule, keep sources and evidence limitations, and redirect retired article URLs to the corresponding chapter. New worked examples are explicitly hypothetical. This consolidation is local only until a new push/publish request. See `docs/series-consolidation/README.md` and `manifest.json`.
