...
| 观察到的情况 | 循环处理 | 为什么 |
| --- | --- | --- |
| 取得新证据，但还缺修订关系 | 在剩余预算内继续查证 | 信息有所增加，仍有明确缺口 |
| 连续得到相同资料与相同缺口 | 换有依据的策略，或报告停滞 | 继续消耗不能自动改善结果 |
| 缺少只能由用户补充的资料 | 等待有效输入 | 反复调用模型不能生成真实资料 |
| 工具请求已发出，写入结果未知 | 转入对账 | 重做可能造成重复副作用 |
| 本轮预算耗尽 | 保存未完成事项并终止本轮 | 资源上限不是业务完成条件 |
| 实际工单通过任务验收 | 报告完成并附资源引用 | 有可检查的交付结果 |

“三次没有新证据就退出”可以是一个初始策略，但不是普遍正确的阈值。探索任务可能需要几次铺垫，确定性查询则可能第一次失败就足以判断缺权限。阈值应在真实任务切片上校准。

循环也不必要求每一步都立即提高某个分数。读取索引页可能只是在为后续证据定位做准备。更有用的要求是：探索必须有范围、有累计预算，并能解释当前动作与剩余缺口的关系。
...
### 检查点保存运行状态，不会替外部服务提交事务

图框架中的持久化容易造成一个误解：有了 checkpoint，就不必再考虑重复执行。LangGraph 文档区分了保存线程运行状态的 checkpointer 与保存跨线程数据的 store；内存实现也不能提供进程重启后的恢复能力。[持久化文档](https://docs.langchain.com/oss/python/langgraph/persistence)

更具体的风险发生在中断恢复。LangGraph 的 `interrupt` 恢复会重新进入中断所在节点，之前的代码可能再次执行。如果把“创建工单”放在节点内的审批中断之前，恢复就可能再次触发创建。[Interrupts 官方说明](https://docs.langchain.com/oss/python/langgraph/interrupts)

本案例应把许可确认放在写入之前，并让写入使用服务端支持的稳定幂等键。即便把审批和写入拆成两个节点，外部写入成功与检查点落盘之间仍可能存在故障窗口；业务资源查询和对账依然必要。图节点完成，不等于模型之外的业务事务已经被完整证明。

长任务还需要固定图和状态 Schema 的版本。部署时重命名节点、删除状态字段或改变分支集合，可能让旧检查点无法按原语义恢复。可以让在途任务继续使用旧版本，或提供显式迁移与重新验收；不要默认新图能无损接管所有旧状态。
