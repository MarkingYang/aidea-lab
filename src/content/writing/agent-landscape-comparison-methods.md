...
<!-- diagram:agent-landscape-comparison-methods-2 -->

```mermaid
block-beta
%% title: 竞品分析矩阵图
 columns 3
 h["实测维度"] a["候选 A"] b["候选 B"]
 r1["版本、环境、权限"] a1["先记录配置"] b1["先记录配置"]
 r2["合格交付与证据"] a2["待同题验证"] b2["待同题验证"]
 r3["人工时间与总成本"] a3["待完整计量"] b3["待完整计量"]
 r4["撤权与恢复"] a4["待故障验证"] b4["待故障验证"]
```

这是一张待填的同题实测矩阵，A／B 代表实际入选候选。未知项不能当作零分，权限与恢复硬条件也不能被低成本抵消。
...
<!-- diagram:agent-landscape-comparison-methods-1 -->

```mermaid
flowchart TB
%% title: 产品选型路线图
 A["第 1—2 天：冻结任务与人工基线"] --> B["第 3—6 天：各候选同题重复运行"]
 B --> C["第 7—8 天：失败复现与恢复测试"]
 C --> D["第 9—10 天：复测与决策记录"]
 D --> E{"硬约束与业务门槛通过？"}
 E -->|是| F["限定任务采用；记录退出条件"]
 E -->|否| G["不采用或缩小范围"]
```

PoC 路线沿用本文两周教学安排。第 9—10 天是复评窗口，不保证届时必然采用；硬约束不通过就保留不采用或缩小范围的结论。
