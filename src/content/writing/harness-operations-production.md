...
<a id="harness-operations-production"></a>
...
这需要明确哪些资源可以重建：代码目录可以从版本与制品恢复，未持久化的内存变量不能；短期凭证恢复时需要重新获取，旧审批也要按当前策略检查。已有[持久任务架构篇](/writing/pi-architecture-deep-dive/#pi-durable-harness-governance)可作为进一步阅读。
...
配套[工作簿](/labs/harness-operations-workbook.md)要求为每个反例写出执行权、操作键和验收证据。[本地实验](/writing/harness-engineering-recovery/#harness-foundations-lab)没有实现多 Worker，因此只能提供副作用恢复的基础，不能当成这些生产性质已经被证明。
