...
[Codex 的固定源码研究](/writing/codex-system-overview/)提供一个具体参照：`CodexThread` 暴露输入与生命周期接口，`Session` 管活跃回合和输入队列，`run_turn()` 在同一回合内组织多次模型请求。启动、接续与恢复是不同输入路径。因此上表的教学对象不能机械映射为厂商同名类，一次 Turn 也不能当作一次模型调用。
...
Codex 的挂起路径说明了交接的具体成本：先刷新历史，再取消任务，停止生产者，最后刷新并关闭写入器，之后才报告关闭完成。未处理输入和交互等待者存在进程内边界，已加载后代检查也不等于原子冻结全部子任务。[源码顺序与限制](/writing/codex-system-overview/)是静态研究，未运行迁移实验；它支持“交接必须包含写入权与未处理输入”的设计推论，不证明主机故障恢复或外部动作幂等。
...
具体组件与组合见 [Agent Runtime 技术选型](/writing/agent-runtime-selection/)：分开比较执行内核、图编排、持久任务服务与沙箱，并按恢复对象、任务场景和故障验收确定采用边界。
