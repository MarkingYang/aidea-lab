...
![请求 ID 关联一次往返，会话 ID 标识采用会话机制的协议交互，业务操作键由应用用于同一业务意图的追踪与对账。](../../assets/illustrations/articles/harness-foundations-mcp-lifecycle.png "图解｜三种标识覆盖不同范围；并非所有传输都要求会话 ID，重连不能证明写入未发生。")
