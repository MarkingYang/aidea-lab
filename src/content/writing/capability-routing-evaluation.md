...
[第三篇](/writing/capability-routing-execution/)让候选变成可执行方案。这篇讨论“凭什么相信它”：不是为路由器增加更多组件，而是为每一层建立可检验的证据。
...
*图 4｜菱形是诊断问题，矩形是排查方向；“是”表示继续定位，不代表上一层已经没有优化空间。多能力任务要检查一条可接受方案的完整覆盖，而非只命中一个工具。*
...
*图 5｜状态图中的箭头表示条件触发的迁移，不是固定调用顺序。“等待授权”不能无限等待；超时、取消和总预算应由运行时约束。图中没有从不确定写入直接跳回重复执行。*
...
至此，我们拥有了架构、检索、执行和验证。但回头看，更多候选、更强精排、更多 Agent 都不必然让结果更好。

最后一篇会回到第一张全景图：当局部最优不等于整体有效时，这套系统应该如何被理解？

[← 第三篇：方案与执行](/writing/capability-routing-execution/) · [第五篇：重新理解路由 →](/writing/capability-routing-synthesis/)
