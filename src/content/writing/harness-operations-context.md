...
如果需要先区分提示词、上下文与运行系统的职责，可以从 [Harness：Agent 架构](/writing/prompt-context-harness-engineering/)进入；本文进一步展开每一步输入的装配机制。
...
<!-- diagram:harness-operations-context-1 -->

```mermaid
flowchart TB
%% title: 数据流图
 A["任务与当前运行状态"] --> F["范围、来源与时效检查"]
 B["检索证据与历史记忆"] --> F
 C["指令与工具说明"] --> F
 F --> S["按相关性与必要性选择"]
 S --> N["整理消息配对与压缩"]
...
上下文数据流是本文装配建议。权限、来源与必要条件先于排序；预算不足时改变读取或分步方案，不能静默删除关键约束。
...
[OpenCode](/writing/opencode-system-overview/)把旧工具输出清理与完整历史摘要分开。前者给结果加清理标记，构建模型输入时替换为占位信息；后者生成摘要，再保留一段近期历史。这说明“记录还存在”和“模型这次能看见”必须分开检查。定位遗漏时，应同时查看持久消息、压缩标记和最终请求。

[Kimi Code](/writing/kimi-code-system-overview/)还在模型输入投影中重排工具结果、补齐缺失结果的占位信息，并记录修复类型。占位信息明确结果未知，不能当成成功或失败凭据。其压缩选段会避免切断工具交换；实测中，近期消息参数设为 4 时可能只原样保留最后 1 条，设为 5 时才保留整个交换所在的尾部。因此，不能只验收“配置了多少条历史”，还应检查实际切点和被省略内容。

这两项研究支持把装配过程拆成四个可检查的动作：选入资料、整理消息结构、缩减内容、序列化请求。每一步都应说明改变了什么。修复结构不等于修复业务结果，减少输入也不等于删除持久数据；这些区别决定出错后应回查哪里。具体提交、代码位置和已运行实验见两篇项目文章。

[Haystack](/writing/haystack-pipeline-architecture/)则提供了另一种具体连接：Retriever 返回 Document，PromptBuilder 按模板保留编号与正文，PipelineTool 将这条流程暴露为一个查询工具。本文关联的实验只把证据组装成上下文，没有调用模型生成答案；这使“检索到”“已经注入”和“被正确使用”保持为三个可分别验证的阶段。
...
提示如何表达任务要求，见 [Prompt Engineering](/writing/prompt-engineering/)；外部证据怎样从入库走到答案核验，见 [RAG](/writing/rag/)。两者分别决定如何使用资料、如何取得资料，再由上下文工程组织本次输入。
