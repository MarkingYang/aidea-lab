...
<!-- diagram:product-work-methodology-1 -->

```mermaid
block-beta
%% title: 产品路线图
 columns 3
 a["0—30 天：明确任务"] b["31—60 天：建立边界"] c["61—90 天：有界委托"]
 a1["窄任务与人工基线"] b1["草稿、权限与评测"] c1["恢复、接管与复用"]
 a2["产物：结果契约"] b2["产物：失败集与记录"] c2["产物：有限自治范围"]
 a3["门槛：成果可验收"] b3["门槛：权限恢复合格"] c3["门槛：质量成本可接受"]
```

产品路线图：按列从左向右推进，沿用本文 0—90 天教学计划。每列同时列出目标、产物和验收门槛；未通过就留在当前阶段修复，已开放范围发生关键退化时收缩权限。日期不构成自动升级条件。
