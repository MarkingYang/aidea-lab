...
<!-- diagram:china-work-agent-showdown-1 -->

```mermaid
block-beta
%% title: 竞品分析矩阵图
 columns 4
 h["比较维度"] k["Kimi Work"] w["WorkBuddy"] d["豆包工作"]
 r1["工作对象"] k1["资料与研究问题"] w1["组织文件与系统"] d1["个人文档与多模态"]
 r2["采用假设"] k2["资料密集型研究"] w2["企业连接与写回"] d2["内容与视觉交付"]
 r3["优先核验"] k3["来源合并与去重"] w3["租户身份与权限"] d3["可编辑性与返工"]
 r4["同题交付质量"] k4["未实测"] w4["未实测"] d4["未实测"]
```

竞品矩阵按本文 2026-09-05 的资料与分析假设整理。企业写回、跨端接管和最终交付质量仍需在指定版本与租户验证，矩阵不提供强弱排名。
...
<!-- diagram:china-work-agent-showdown-2 -->

```mermaid
flowchart TB
%% title: 产品采用路线图
 A["固定资料、任务和人工基线"] --> B["只读试点：核验来源与口径"]
 B -->|证据合格| C["制品试点：检查编辑性与复算"]
 C -->|文件可用| D["受控写回：批准、权限、结果查询"]
 D -->|撤权与故障测试通过| E["有限推广：成本、维护与退出"]
 D -->|失败或未知| F["暂停写回并对账"]
```

办公 Agent 的采用路线：先确认文件交付价值，再开放经批准的写回。此图是团队采用建议，阶段由验收证据推进，没有厂商发布时间承诺。
