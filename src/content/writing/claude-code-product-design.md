...
<!-- diagram:claude-code-product-design-1 -->

```mermaid
flowchart TB
%% title: 产品路线图
 A["先验证：用户看懂目录、权限与停止原因"] --> B["优先完善：Diff、测试与证据缺口"]
 B -->|单任务交付可验收| C["再验证：并行任务隔离与接管"]
 C -->|审查成本可控| D["扩展：跨端状态与持续任务"]
 B --> E["独立探索：分享视图与隐私模式"]
 E -->|访谈和脱敏测试| F["决定是否纳入版本"]
```

这是一条从本文问题推导的产品改进路线，不是 Anthropic 的官方计划。先减少目标与证据误解，再验证并行接管；分享脱敏需要单独研究和验收。
