...
![Prompt 规定怎样判断字段，Context 提供有效公告与待核验记录，Harness 检查许可、执行创建并回读验收。](../../assets/illustrations/engineering-scopes.png "职责图解｜以资料核验为例；发布日与生效日是不同字段，三种职责共同完成任务。")
...
固定任务、模型设置和数据快照：Prompt 看字段判断与结构合法性；Context 看必要证据是否进入请求；Harness 看实际资源、异常恢复和预算。保留越权提案与实际越权执行两种记录，不能仅以“最终没有删除”抹平行为错误和门禁成功的区别。

每次只改变一层，并保留未参与调试的样本。通过率、成本分母和重复运行方法统一采用[Agent 评测指标](/writing/agent-evaluation-metrics/)。完成这项练习应留下三份产物：任务契约、上下文来源清单、执行与验收记录；它们分别支持三层的结论。
