...
购物车案例说明职责，下面用已有的“核验公告生效日期”实验把设计连接起来：读取公告和目录，形成冲突提案，批准后创建工单，再查询字段。公告发布日期为 9 月 1 日、生效日期为 9 月 15 日，目录误记为 9 月 1 日。这是固定教学数据，不是真实业务成果。

同一任务依次产生不同性质的对象：任务入口给出可信目标；两次读取产生观察；模型替身依据观察形成提案；批准允许进入下一步；资源端再次检查许可并写入；运行器查询工单，对照原始日期与来源验收。**提案、批准、写入、验收分别改变不同事实，不能压缩为一个 success 标记。**

| 为什么需要这个边界 | 如果省略，会失去什么 | 已有项目提供的具体参照 | 当前证据能支持什么 |
| --- | --- | --- | --- |
| 任务要求与模型提案分开 | 模型可以把自己的输出当成正确目标 | Codex 的输入接纳与回合控制；Mini Harness 的可信字段校验 | 工单反例能检出非法提案，不等于检验任意自然语言理解 |
| 历史记录与本次输入分开 | 压缩或消息补齐可能被误认为业务事实 | OpenCode 的输出清理、Kimi 的历史投影、Codex 的 `for_prompt` | 有固定源码与部分模块实验；未证明摘要质量 |
| 编排状态与业务资源分开 | 检查点存在就被误解为写入只发生一次 | LangGraph 的检查点、Temporal 的历史与 Activity | 已有局部恢复实验；幂等仍由业务资源负责 |
| 执行许可紧邻资源写入 | 等待时已撤权，旧批准仍可能执行 | OPA 的决策与执行点分工；本地资源事务检查 | 工单实验验证策略行，不是身份认证或分布式撤权 |
| 操作键与尝试次数分开 | 每次重试都可能创建新资源 | Mini Harness 的查询与幂等创建 | 响应丢失后复用键保留一条；换键可产生两条 |
| 工具结果与验收分开 | 成功创建了错误字段，也被宣布完成 | 工单独立查询与可信字段对照 | 可以检出预置错误资源，不是所有任务的通用裁判 |

项目并不因为出现在同一表格就应全部接入。它们分别说明不同实现选择：[Codex](/writing/codex-system-overview/)强调交互输入与持续回合，[LangGraph](/writing/langgraph-runtime-architecture/)表达依赖和状态合并，[Temporal](/writing/temporal-durable-execution/)处理持久工作流。是否组合，要先确定哪个组件拥有等待、重试和最终写入权。

本轮在同一工单核心上运行了八组“保留规则／改变规则”对照，共十六次执行。预算为零时删除检查仍会读一次；已有工单字段错误时省略核验会误报成功；提交后丢响应再更换操作键，会留下两张工单。这些结果说明规则在固定反例中的作用，不能据此给框架排名。完整输入、干预与结果见[架构选型中的对照解释](/writing/harness-architecture-selection/#why-these-boundaries)及[实验包](/labs/architecture-rationale.zip)。

由此可以形成可推翻的设计判断：当前任务只有两次读取、一次批准和一次创建，显式循环足够；接受自行维护状态的成本。分支和暂停难以维护时再比较图编排，长期跨服务等待时再比较持久工作流。真实模型、真实权限、多租户与跨机器恢复仍需单独验证。

## Agent 架构设计：从约束推出边界

前面的八项职责回答系统需要做什么。真正决定架构形态的，是任务持续多久、会修改什么、允许怎样失败，以及交付需要哪些证据。不能仅按“有规划、工具、记忆”画模块图，再把剩余问题交给 Runtime。

以销售月报为例：输入可能更新，生成过程可能中断，输出只能保存草稿，金额必须可复算。这四个约束分别要求来源版本、可恢复运行身份、执行权限和独立验收。设计理由应能沿“约束—机制—代价—验证”走完，而不是停在组件名称。

| 约束与反例 | 选择的职责边界 | 接受的代价 | 用什么验证 |
| --- | --- | --- | --- |
| 输入会变，昨天的结论今天难以复核 | 摄取固定来源版本，运行绑定输入清单 | 保存与治理版本的成本 | 改写源表后不能继续用旧验收结论 |
| 模型可以探索，但不能自行扩大写入范围 | 模型提出动作，确定性执行层检查权限与预算 | 更多拒绝分支、可能需要用户补充信息 | 合法与越权动作成对测试 |
| 保存已经发生，响应却丢失 | 业务动作身份与调用尝试分开，未知结果先对账 | 操作键、查询与恢复逻辑 | 丢失响应不能导致重复交付 |
| 报告文字正确，实际文件可能被覆盖 | 产物版本与独立验收绑定，交付器读取同一版本 | 采集和存储开销 | 验收后替换文件必须被发现 |
| 修复有效，但可能只记住开发题 | 离线评测输入与参考答案分开，按来源家族留出 | 样本策展和版本维护 | 相似原表不能跨开发与留出集泄漏 |

这些选择不要求五个微服务。低并发、短任务可以先在同一进程里维护模块接口，用本地持久存储保存关键状态；有不同信任域、资源配额或故障隔离需求时，再将工具执行器、任务调度或评测运行器拆开。拆服务会增加通信、授权传播和部分失败，必须说明它换来了哪种独立性。

### 自由度与控制权分别放在哪里

确定性部分负责维持不变量：什么输入可读、什么动作可执行、预算还剩多少、当前谁拥有执行权、哪些成果已经验收。模型负责不确定的任务判断：选择哪份相关资料、提出哪一步操作、如何组织解释。工具输出与用户文件仍是输入资料，不能因为出现在上下文里就获得修改系统规则的权力。

流程稳定且依赖明确时，可以把阶段表达成 Graph；路径变化大、单步反馈足够时，Loop 可能更直接；长时间等待和跨进程恢复再推动持久工作流。这些是控制结构的选择，不会代替权限、数据血缘和验收。[架构选型](/writing/harness-architecture-selection/)与 [Multi-Agent 选型](/writing/multi-agent-architecture-selection/)分别讨论何时增加编排和协作机制。

### 在线执行与离线改进怎样相接

<!-- diagram:prompt-context-harness-engineering-data-eval -->

```mermaid
flowchart TB
%% title: 架构、数据与评测反馈图
 T["任务契约与配置版本"] --> R["在线 Runtime"]
 S["受控输入快照"] --> R
 R --> X["受约束工具执行"]
 X --> E["动作证据与候选成果"]
 E --> V["任务验收"]
...
在线验收决定当前任务能否交付；离线评测比较未来版本在一组固定任务上的表现。运行证据经过筛选与复现后进入数据集，候选版本通过发布判断才改变后续任务配置。图中的反馈不能直接改写在途 Run 的策略和模型版本。

<!-- /diagram -->

这条反馈链不能压缩成“把所有日志回流训练”。[Agent 数据架构](/writing/agent-data-architecture/)负责说明来源、运行、成品和样本怎样关联；[评测系统](/writing/agent-system-evaluation-research/)负责说明参考答案怎样隔离、缺失试次怎样计数、哪些结果足以阻止发布。数据保留规则也会反向约束架构：无法长期保存原始内容时，就应明确哪些历史任务只能查验记录、无法完整重跑。

一个可评审的 Agent 架构至少应该让读者沿正常路径和一次失败路径走通：谁接收输入，谁提出与许可动作，谁写权威状态，哪里保存成果，谁判断完成，哪个模块负责未知结果。若还需要靠一句“框架会处理”才能跨过其中一段，就仍有设计责任没有落地。
