...
若需要继续选择具体 SDK、任务服务和代码执行环境，参见 [Agent Runtime 技术选型](/writing/agent-runtime-selection/)。本文保留三种控制架构的历史实验；该选型篇另行区分组件层次、证据级别与目标部署的待测项。
...
<a id="why-these-boundaries"></a>

## 用反例解释为什么需要这些边界

三个控制引擎都能通过契约检查，并不能证明每个机制的贡献。为进一步解释设计，2026-09-08 对同一个工单核心运行了八组配对反例：控制组保留原实现，变体每次改变一项规则或调用方式，其他场景条件保持一致。各次使用独立的临时 SQLite；七组只在内存中修改核心源码，第八组改变重试键，不覆盖原代码和历史结果。[可复现实验包](/labs/architecture-rationale.zip)

| 相同输入或故障 | 保留规则的结果 | 改变规则的结果 | 解释的设计责任 |
| --- | --- | --- | --- |
| 决策预算为 0 | `exhausted`，模型替身调用 0 次 | 删除预算检查后读取一次，调用数为 1 | 预算必须在行动前落实 |
| 状态版本为 999，提案已批准 | `migration_required`，0 张工单 | 忽略版本后继续创建，1 张 | 未知状态不能直接按当前语义运行 |
| 提案夹带 `approved` 字段 | 拒绝，0 张工单 | 省略提案校验后进入等待，仍为 0 张 | 校验阻止非法提案进入后续流程；独立批准仍保留保护 |
| 同操作键下已有错误日期的工单 | 核验失败 | 只看资源存在便误报成功 | 资源存在与字段正确是两项事实 |
| 前置许可检查后、写入前撤权 | 资源端拒绝，0 张 | 删除资源事务内检查后写入，1 张 | 前置检查不能代替提交处的当前许可 |
| 相同操作键，第二次参数不同 | 拒绝参数冲突 | 删除参数绑定检查后返回接受，但资源仍是旧参数 | 仅防重复不足以保证请求含义一致 |
| 提交后丢响应，再收到取消 | 报告 `cancelled_with_effect`，1 张 | 把状态简化为 `cancelled`，仍有 1 张 | 取消的呈现必须保留已发生影响 |
| 提交后丢响应，查询后重试 | 复用原键查到资源，1 张 | 重试换新键，查询未命中并再创建，2 张 | 业务操作身份应跨尝试保持稳定 |

八组、十六次执行均符合预先写下的预期。变体是故意违反契约的反例，不是另外八种可采用架构。第三组尤其说明多层约束的价值：删掉提案校验并没有直接获得批准，不能把观察夸大成“模型已越权写入”。第五组直接调用资源接口模拟两个检查之间的撤权窗口，不是同时运行两个真实用户；权限仍由教学策略行表示。

这些实验执行真实 Python 控制逻辑与 SQLite 写入，模型决策仍是固定响应，没有运行 A/B/C 引擎、Codex 或外部 SaaS。它们补充的是业务契约的局部因果解释；前文 72 次引擎检查是另一份历史实验，不能相加后称为真实模型成功率。

从反例到选型还差一步：明确这个失败是否存在于目标业务。只读问答可能不需要写入去重；确定字段的短流程可能不需要模型；不跨进程等待的任务未必需要持久工作流。保留机制的依据应是它维护了哪条实际不变量，以及维护成本是否值得承担。
