...
任务题目相同，不代表两次评测条件相同。源表、模型配置、工具定义、系统提示、检索索引、评分规则和允许重试次数都可能变化。开始执行前应冻结清单，并把**计划试次**单独登记，避免运行崩溃后只有成功写出的结果还能进入统计。

```yaml
experiment_id: monthly-report-fixture-v1
# 下列值是本文示例，不是某个平台的标准字段。
dataset_version: sales-fixture-v1
input_manifest: sha256-of-snapshots
application_version: candidate-commit
model_config: pinned-config-or-explicitly-unavailable
context_and_tools_version: context-tools-v1
grader_version: report-contract-v1
planned_trials: [task-a-run-1, task-a-run-2, task-b-run-1, task-b-run-2]
timeout_policy: record-before-stop
retry_policy: record-every-attempt
reference_visibility: grader-only
```

这里的示例摘要是占位符，接入实现时必须换成真实摘要与可解析版本。记录模型名称也不能保证服务内部完全不变；复现不了的条件要列为限制。补记缺失配置不能把一次历史运行变成已经受控的对照实验。

LangSmith 将数据集上的某个应用版本评测组织为 Experiment，并提供样本版本管理。借用这个思路，本文进一步要求实验绑定评分器版本与计划试次表，具体字段仍由应用定义。[Evaluation concepts](https://docs.langchain.com/langsmith/evaluation-concepts)

<!-- diagram:agent-system-evaluation-research-manifest -->
...
汇总同时读取计划表和判定记录，因此没有返回结果的试次仍然存在。Agent 只收到任务输入，评分器另外读取参考答案。独立目录是工程组织方式；真实隔离仍需文件权限、凭据、进程或沙箱边界，不能仅靠调用约定。

<!-- /diagram -->

输入家族、数据保留和参考答案的流转见 [Agent 数据架构](/writing/agent-data-architecture/)。尤其需要检查同一原始工作簿衍生的多道题是否跨越开发与留出用途；仅做逐文件哈希去重，无法识别改写题目的泄漏。

## 判定保留三种状态，发布规则再决定如何处理

本文采用 `pass / fail / unknown` 作为教学判定，不是框架或协议规定的状态。它们分别表示：所需检查通过、已证实违反契约、证据不足以完成判断。判定同时保留具体失败与缺失项，不用一个状态抹掉另一类事实。

| 观察到的情况 | 判定 | 原因 |
| --- | --- | --- |
| 数字正确、来源和成品绑定有效、行为审计完整且符合约束 | pass | 在本量表覆盖范围内验收通过 |
| 金额错误或已观察到未经授权外发 | fail | 存在确定的契约违反 |
| 成品缺失、审计不完整或采集后文件被替换 | unknown | 不能证明当前待交付成果符合要求 |
| 已观察到外发，同时成品缺失 | fail，并保留缺失项 | 证据不全不能撤销已经成立的违规 |

Agent 在规定预算内没有完成，是任务结果问题；评分服务故障使成品无法被判断，是测量问题。两者都不能算通过，但应分开记录。若确认是运行器基础设施错误而决定重跑，要保留原记录、重跑关系和原始计划数；不能事后静默剔除失败条件来抬高分数。对可确认的任务超时也应按预先契约判失败，不能全塞进未知。

新增[本地实验包](/labs/agent-data-evaluation.zip)用四个固定候选成品得到 `pass、pass、fail、unknown`。已验收比例是 `2 / 4 = 50%`；如果只对已知判定计算 `2 / 3`，必须标成“已判定样本中的通过比例”，同时显示未知占 `1 / 4`。后者不能冒充整体成功率。

四次运行的人为成本分别为 `1、1、2、1` 单位，因此每个验收通过成果分摊成本为 `(1 + 1 + 2 + 1) / 2 = 2.5` 单位。如果有试次漏记且成本也未知，实验程序返回成本不可用，而不是把它算成零。这些数值只验证汇总口径，未调用模型、没有真实费用，更不能用于估计某个 Agent 的业务表现。

## 先校准评委，再解释版本差异

已有错误成品还应形成评委的校准集：人工核定“可接受／不可接受”，再观察评分器放过多少坏成品、误拒多少好成品，以及多少无法判断。开放质量评委还应对难例保留独立复核和分歧依据。评委自己重复运行的波动，不能算成被评 Agent 的波动。

例如一组**假设数据**有 20 个不可接受成品和 80 个可接受成品。评委错放 4 个坏成品、误拒 8 个好成品，则总体正确率为 88%，但坏成品放行率是 20%。只报告 88% 会隐藏交付风险。这里是算术示例，没有运行模型评委或人工标注实验；选择何种错误容忍度仍取决于任务。

版本比较则应在同一任务集合和可比环境中配对。若 A 与 B 都跑 100 个试次，但 A 集中跑简单任务、B 集中跑难任务，总通过比例没有直接可比性；即使题目一致，某任务多跑几十次也会改变试次加权结果。应同时说明每任务重复次数、任务权重和切片分布。

任务重复试次帮助了解该任务的随机波动，不会自动增加独立任务的覆盖。需要统计区间时，应根据采样设计以任务或来源家族为单位处理相关性；简单把同一工作簿衍生的试次当成独立样本，会夸大证据量。样本量不足时直接展示配对结果、失败类型和不确定性，比伪精确的排名更有用。

发布判断也要写成明确动作：存在确认的禁止项则阻止相应能力开放；存在关键未知则补证或缩小结论；质量提升但延迟或人工成本退化则按事先约定的取舍决策。实验包只实现“全部计划试次通过才进入下一检查”的教学门槛，不能据此自动批准生产发布。
