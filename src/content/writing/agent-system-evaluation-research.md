...
*图 1｜任务契约如何通过证据进入发布决策。矩形表示模块或步骤，圆柱表示存储，菱形表示判断；实线表示主路径，虚线表示约束、信息支撑或反馈。图为教学抽象，不代表全部实现细节。*
...
原长文中无法稳定核查的跨榜单排名、无实测依据的对比量表与未注明实验条件的百分比不再保留。这里不做产品总冠军榜，而是让读者搭出自己的验证链。
