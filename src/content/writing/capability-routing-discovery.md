...
[上一篇](/writing/capability-routing-at-scale/)把路由分成发现、选择、执行和验收。这一篇只展开“发现”：如何把上万个能力转成一小组值得进一步比较的候选。

继续用会议任务。用户说“提取客户讨论的结论和后续动作”，目录却写“会议纪要”；两个 Skill 摘要相同，正文却分别要求本地整理和外部上传。这两种错误的来源不同：前者搜不到，后者分不清。
...
*图 2｜实线是在线处理步骤，虚线是索引或详情供给；圆柱是存储，菱形是准入判断。精排只处理已召回候选，不能补回被遗漏的能力。*
...
发现层的职责是提供足够好的选择范围。它既不能恢复已经漏掉的能力，也不能证明一个相关工具独立完成得了任务。

如果检索同时返回“会议总结 Skill”“创建文档工具”和“会议助理 Agent”，接下来该比较什么？第三篇会把候选组织成方案。这个变化，也是理解最终整套系统的关键一步：我们正在从搜索对象，走向选择行动。

[← 第一篇：系统全景](/writing/capability-routing-at-scale/) · [第三篇：从候选列表到可执行方案 →](/writing/capability-routing-execution/)
