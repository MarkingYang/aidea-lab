...
stateDiagram-v2
  [*] --> Prepared: 意图已保存
  Prepared --> Unknown: 派发后未确认结果
  Unknown --> Confirmed: 查到匹配资源
  Unknown --> Prepared: 权威确认未执行且允许重试
  Unknown --> Reconcile: 查询仍无法确定
  Reconcile --> Unknown: 有效事件或有界重查
  Confirmed --> Verified: 独立验收通过
  Confirmed --> Repair: 资源不符合目标
  Repair --> Compensated: 获得授权后执行补偿
  Verified --> [*]
  Compensated --> [*]
```

*图 2｜设计状态图：未知结果不能直接等同失败；补偿是针对既有影响的新操作，需要自己的授权与验收。*
