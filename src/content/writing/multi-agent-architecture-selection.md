...
![字段固定的三份材料可通过并行工具核对；各分支都需要独立多步查证时，才考虑独立上下文与 Agent，并由协调者汇总验收。](../../assets/illustrations/articles/multi-agent-architecture-selection.png "图解｜并行调用与多 Agent 是不同选择；拆分后必须核算协调成本与交付责任。")
