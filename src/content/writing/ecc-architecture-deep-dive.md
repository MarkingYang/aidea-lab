...
ECC 是给现有 Coding Agent 安装规则、技能和 Hooks 的配置与资产系统；它用安装计划和所有权记录替代手工复制、覆盖与清理配置文件。

适合统一团队开发环境；它不是新的模型循环，也不能把本地 Hook 等同于不可绕过的组织安全边界。
...
```mermaid
flowchart TB
  E["入口：setup / install 与目标选择"] -->|任务或调用| C["核心：计划、依赖解析与 Apply"]
  C -->|调用 / 加载| A["适配：Claude / Codex 等宿主适配"]
  A -->|请求 / 读写| I["基础设施：配置目录、资产文件与运行日志"]
  I -->|结果 / 状态| A
  A -->|规范化结果| C
  C -->|回答 / 产物| E
```
...
```mermaid
sequenceDiagram
  participant U as 安装命令
  participant C as 安装内核
  participant T as 目标目录
  U->>C: 选择 profile 与 target
  C->>C: 生成依赖和文件计划
  C->>T: 校验路径、冲突与所有权
  T-->>C: 应用计划并记录安装状态
  C-->>U: 返回实际安装结果
...
*图 3｜ECC 位于模型 Harness 之外，以资产、安装器和 Adapter 把工程意图投射到不同平台。*
...
| --- | --- |
| `--profile` | 按使用场景选择安装集合 |
| `--target` | 选择目标宿主，避免混用路径 |
| `--modules` | 显式选择模块；与 profile 方案按 CLI 规则使用 |

其余选项见 [配置参考](https://github.com/affaan-m/ECC/blob/22e8cf01d0b54719b3a49002fab2ccbda4ff5b9e/scripts/install-plan.js)。
...
截至 2026-09-06，2026-08-08 至 09-06 UTC 的抽样取得 至少 30 条默认分支提交（上限 30 条）。见 [提交记录](https://github.com/affaan-m/ECC/commits/main/)。

Issue 取 08-08 至 08-30 UTC 创建的最近最多 3 条非 PR 条目，排除机器人和提问者自答；3 条中 0 条观察到维护者文字回复。 样本：[#2910](https://github.com/affaan-m/ECC/issues/2910)、[#2909](https://github.com/affaan-m/ECC/issues/2909)、[#2906](https://github.com/affaan-m/ECC/issues/2906)。小样本不代表 SLA，“未观察到”也不代表其他渠道无人处理。

固定快照许可证：[MIT](https://github.com/affaan-m/ECC/blob/22e8cf01d0b54719b3a49002fab2ccbda4ff5b9e/LICENSE)。允许商业使用，但分发时仍须保留版权与许可声明。

同仓维护多宿主安装与 Hook 适配；AgentShield 是独立项目，不能把其路线图算成 ECC 已有能力。
...
| 5 | [tests/scripts/install-plan.test.js](https://github.com/affaan-m/ECC/blob/22e8cf01d0b54719b3a49002fab2ccbda4ff5b9e/tests/scripts/install-plan.test.js) | 安装计划用例 |
