...
需要历史问答和纠错时，可在保留期限内追加原始事件，并允许派生画像更新；高层结论记录来源、有效时间和生成版本。追加策略不免除删除与到期清理义务。
...
字段与上例一致：7 月 2 日的消息在 7 月 3 日入库，搬家从 6 月 10 日生效。`supersedes` 指向被替代的上海住所记录；这是自定义格式。
...
纠错验收应覆盖原始记录、当前事实及派生摘要；具体删除传播见[记忆治理](/writing/agent-memory-governance/)。
