---
title: Harness 接入与实战（一）：Mini Harness 的模块、接口与默认选型
...
要搭一个能完成任务的 Harness，第一步是画清模块，以及它们交换什么。本系列采用一个具体起点：**单用户、单 Worker、Python、本地执行、允许重启恢复**。在这个范围内，优先选择少量显式函数、严格数据校验和 SQLite；出现明确瓶颈后再换组件。
...
“最佳”必须有适用条件。下面是面向本系列场景的工程建议，不是经过统一基准测试的工具排行榜。
...
本系列的第一块可运行切片是：给定 `source` 与 `title`，创建一张本地待核验工单，重启后同一业务操作仍只对应一张工单。

这个任务的执行路径已经确定，因此它是**单动作工作流**：模型负责产生结构化提案，运行器执行固定的查询、创建和验收步骤。实际业务中，如果字段已经完全确定，可以直接调用创建函数。这里保留模型适配，是为了单独练习生成结果如何进入受控执行链路。

在此基础上，真正需要 Agent 决策的扩展任务可以是“读取三份材料，比较冲突，生成待核验问题，再创建工单”。这时模型要根据工具结果选择下一步，才需要多轮循环；第二篇会给出循环接口与消息往返，实验包尚未实现该扩展。
...
下一篇：[模型与循环](/writing/harness-integration-model/)。
