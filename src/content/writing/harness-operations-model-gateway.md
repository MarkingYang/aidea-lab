...
<a id="harness-operations-model-gateway"></a>
...
备用模型必须满足当前任务的输入和工具能力。切换有独立预算，HTTP 成功不代表任务可继续。
## 适配器需要保留必要差异
...
[LiteLLM 项目研究](/writing/litellm-gateway-architecture/)给出具体反例：同组重试会再次进入部署选择，未必重用同一个部署；普通备用组与上下文超限备用组也有不同配置路径。在 1.100.0 的受控实验中，同时配置这两种备用组后，超窗错误直接进入专用组。实验只验证选路，没有验证备用模型实际能容纳多少上下文。

因此，策略表需要同时记录“错误类别、允许的目标集合、尝试上限”，不能只填写一个备用模型名称。权重与可用性也应分开：该版本的 `simple-shuffle` 在候选全部零权重且无其他正权重字段时仍会随机选出部署，停用目标应使用明确的禁用配置。流式输出已经产生内容后的切换还有独立边界，不能承诺任意位置无缝续写。
