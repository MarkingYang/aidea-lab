...
export const stageLabel = (index: number, length: number) => index === 0 ? '建立全景' : index === length - 1 ? '回到整体' : '拆解与实践';
...
export async function getKnowledge() {
  ...
  const series = seriesDefinitions.map(definition => {
    ...
    if (definition.articles.length < 3) throw new Error(`Series needs overview, practice and synthesis: ${definition.id}`);
