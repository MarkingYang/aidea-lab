...
`src/data/knowledge.json` 是知识域、专题与文章归属的唯一运行注册表；`src/data/content-redirects.json` 保存旧地址的直接迁移。运行：
...
检查覆盖全部发布文章的唯一归属、标题和摘要、写作内链、图注、代码围栏、标题层级、阅读时间及迁移目标。专题可以只有一篇，不要求“总—分—总”，也不为满足篇数拆文。阅读时间按中文、英文、图表、表格和代码估算，不限制文章必须短于十分钟。

浏览器检查使用现有 Playwright 与 Chrome：设置 `BLOG_PLAYWRIGHT_ROOT` 为包含 Playwright 的 `node_modules` 路径，`BLOG_BASE_URL` 为本地站点地址，运行 `node scripts/knowledge-browser-check.mjs` 与 `node scripts/graph-browser-check.mjs`。检查目录展开、搜索、文章导航、图表、图谱交互和移动端布局。

自动检查不能判断选题是否重复。内容 Review 还应检查每篇是否拥有独立问题、证据或实例，是否与其他篇重复表达同一结论；新增内容先确定归属，再决定是否值得独立成篇。
