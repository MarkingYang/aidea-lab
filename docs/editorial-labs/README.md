...
`series.json` 记录各轮工程写作的系列及阅读顺序，并用 `priority` 标记编辑优先级。在仓库根目录运行：
...
前者检查已注册文章的优先级、知识目录顺序、总—分—总闭环、前后导航、跨系列链接、站内文章链接、标题与摘要唯一性、图注、代码围栏、标题层级与阅读时长。主阅读路径按中文约 350 字/分钟、英文约 190 词/分钟估算，并计入图表、表格和代码理解成本；折叠的证据附录与资料列表作为可选阅读，不计入主路径，动手实验另计。所有文章的标注与估算都不得超过 10 分钟，标注也不能明显低估正文。

这套检查只负责客观下限，不能替代内容判断。Review 仍需先读每个系列的首篇与终篇，确认首篇只搭地图、终篇确实由中间证据推出新认识；再抽查实践篇是否给出可以执行的任务、产物和验收方式。

浏览器 QA 需要已安装的 Playwright 和 Chrome，以及运行中的本地站点。设置 `EDITORIAL_PLAYWRIGHT_ROOT` 为本机现有 Playwright 所在的包目录再运行同一脚本；`EDITORIAL_BASE_URL` 默认 `http://localhost:4321`。脚本检查每篇图表与公式渲染、拦截超过单屏阅读上限的流程图，并检查各系列入口的移动端页面溢出。可选 `EDITORIAL_SCREENSHOTS` 指向已存在的截图目录；不会自行安装浏览器或依赖。
