...
## 第六组：三个 Agent Memory 项目纵向研究

横向的“Agent 记忆设计”五篇继续承担统一生命周期与方案关联，不再同时承担三个仓库的源码教学。Mem0、OpenViking 与 TencentDB Agent Memory 分别新增五篇“总—分—分—分—总”系列：

| 独立系列 | 中间三篇的数据主线 | 终篇回到的问题 |
| --- | --- | --- |
| Mem0 源码研究 | ADD-only 写入 → 多信号检索 → 生产边界 | 短接口隐藏了哪些应用责任 |
| OpenViking 源码研究 | 分层目录 → 层级检索 → Session 与权限 | 上下文文件系统如何维护结构 |
| TencentDB Agent Memory 研究 | Proxy 接入 → L0-L3 → 资产与 Loadout | 团队经验何时获得行动权 |

三个系列固定到 2026-09-05 核查的官方仓库提交；Mem0 OSS 与 Platform、OpenViking 已实现与规划能力、TencentDB Team Memory Beta 与稳定能力分别描述。知识图谱形成一个横向比较圆和三个纵向项目圆，以正文引用连接具体机制，而不是只依赖共享标签推断关系。
