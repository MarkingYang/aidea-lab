...
const slugs=['agent-runtime-selection','agent-runtime','harness-architecture-selection'];
...
try {
 for(const width of [1440,390]) {
  ...
  for(const slug of slugs) {
   ...
   for(const theme of ['light','dark']) {
    ...
    if(slug==='agent-runtime-selection') {
     await page.locator('.mermaid-figure').last().screenshot({path:`/tmp/aidea-runtime-selection-${width}-${theme}.png`});
    }
   }
  ...
  await page.goto(`${base}/writing/agent-runtime-selection/`);
 ...
 fs.writeFileSync('docs/runtime-selection/browser-results.json',JSON.stringify({date:'2026-09-09',results,new_diagrams:4,interactions:['zoom','reset','fullscreen','export'],internal_links:'HTTP 200 and target anchors checked',page_errors:[]},null,2)+'\n');
