...
const base=process.env.BLOG_BASE_URL || 'http://127.0.0.1:4409';
const slugs=['agent-data-architecture','prompt-context-harness-engineering','agent-system-evaluation-research'];
...
try {
 for(const width of [1440,390]) {
  ...
  for(const slug of slugs) {
   ...
   for(const theme of ['light','dark']) {
    ...
    if(slug==='agent-data-architecture') {
     await page.locator('.mermaid-figure').last().screenshot({path:`/tmp/aidea-data-state-${width}-${theme}.png`});
    }
   }
  ...
  await page.goto(`${base}/writing/agent-data-architecture/`);
 ...
 fs.writeFileSync('docs/agent-data-evaluation/browser-results.json',JSON.stringify({date:'2026-09-09',results,new_diagrams:6,interactions:['zoom','reset','fullscreen','export'],internal_links:'HTTP 200 and target anchors checked',page_errors:[]},null,2)+'\n');
 console.log('PASS 12 article/viewport/theme combinations; links and new diagram interactions');
