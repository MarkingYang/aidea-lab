...
const manifest = JSON.parse(fs.readFileSync('docs/article-diagrams/manifest.json','utf8'));
const base = process.env.BLOG_BASE_URL || 'http://localhost:4321';
const shots = process.env.BLOG_DIAGRAM_SCREENSHOTS || '/tmp/aidea-diagram-screenshots';
...
try {
 ...
 fs.writeFileSync('docs/article-diagrams/browser-results.json',JSON.stringify({results,screenshotDirectory:shots,visualReview:'pending',publication:'local_unpublished'},null,2)+'\n');
